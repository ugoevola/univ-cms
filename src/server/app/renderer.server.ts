...
const DIST_FOLDER = join(process.cwd(), 'dist/renderer');
...
export class RendererServer {
  ...
  static clientInstance: RendererServer;

  private RENDERER_PORT = Config.get().RENDERER_PORT || 4100;
  private logger = WinLogger.get('renderer-server');

  static bootstrap(app: INestApplication) {
    return new Promise((resolve) => {
      this.clientInstance = new RendererServer(app);
  ...
  private initServer(done) {
    ...
    if (Config.get().RENDERER_PROXY_ACTIVATED) {
    ...
    app.get('/views/*', async (req: any, _res: any, next: any) => {
      ...
      req.BASE_URL = `http://localhost:${this.RENDERER_PORT}`;

      const contentService = this._app.get(ContentService);
      const content = await contentService.findByReference('Fe3sc2BTU');
